...
class ListsSuite {
  ...
  @Test def `sum of a few numbers (10pts)`(): Unit = {
    ...
    Assert.assertEquals(3, Lists.sum(List(1, 2, 0)))
    Assert.assertEquals(1, Lists.sum(List(-1, 2, 0)))
  }

  @Test def `sum of an empty list`(): Unit = {

    Assert.assertEquals(0, Lists.sum(List()))
  }

  @Test def `max of a few numbers (10pts)`(): Unit = {

    Assert.assertEquals(1, Lists.max(List(-10, 0, 1)))
    Assert.assertEquals(15, Lists.max(List(-150, -1400, 15, 13)))
  }

  @Test def `max throws an exception if its argument is an empty list`(): Unit = {
